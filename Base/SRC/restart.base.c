...
static int restart_soft_locking_@(pre)primme(int *restartSize, @(type) *V, @(type) *W, int nLocal,
   ...
   if (V == NULL) {
      ...
      return max(max(max(
                  ...
                  Num_reset_update_VWXR_@(pre)primme(NULL, NULL, nLocal, basisSize, 0, &t,
                     *restartSize, 0, NULL,
                     &t, 0, *restartSize, 0,
...
int Num_reset_update_VWXR_@(pre)primme(@(type) *V, @(type) *W, int mV, int nV, int ldV,
   ...
   if (V == NULL || reset == 0) {
      return Num_update_VWXR_@(pre)primme(V, W, mV, nV, ldV, h, nh, ldh, hVals,
         X0, nX0b, nX0e, ldX0,
         X1, nX1b, nX1e, ldX1,
   ...
   Num_update_VWXR_@(pre)primme(V, NULL, mV, nV, ldV, h, nh, ldh, NULL,
         X0, nX0b, nX0e, ldX0,
         X1, nX1b, nX1e, ldX1,
         evecs?&evecs[ldevecs*evecsSize]:NULL, nX2b, nX2e, ldevecs,
         NULL, 0, 0, 0,
   ...
   matrixMatvec_@(pre)primme(X0, mV, ldX0, Wo, ldWo, 0, nWoe-nWob, primme);
 
   /* R = Y(nRb-nYb:nRe-nYb-1) - X(nRb-nYb:nRe-nYb-1)*diag(nRb:nRe-1) */
   for (j=nRb; j<nRe; j++) {
      Num_compute_residual_@(pre)primme(mV, hVals[j], &X0[ldX0*(j-nX0b)],
            &Wo[ldWo*(j-nWob)], &R[ldR*(j-nRb)]);
      if (Rnorms) {
         @(type) ztmp;
         ztmp = Num_dot_@(pre)primme(mV, &R[ldR*(j-nRb)], 1, &R[ldR*(j-nRb)], 1);
   ...
   if (rnorms) for (j=nrb; j<nre; j++) {
      ...
      for (i=0; i < mV; i+=m, m=min(m,mV-i)) {
         @(type) ztmp;
         Num_compute_residual_@(pre)primme(m, hVals[j], &X0[ldX0*(j-nX0b)],
               &Wo[i+ldWo*(j-nWob)], rwork);
         ztmp = Num_dot_@(pre)primme(m, rwork, 1, rwork, 1);
         rnorms[j-nrb] += *(double*)&ztmp;
      }
   }
