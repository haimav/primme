...
#include "locking_private_@(pre).h"
...
int restart_locking_@(pre)primme(int *restartSize, @(type) *V, @(type) *W,
   ...
   int reset, double machEps, @(type) *rwork, int rworkSize, int *iwork, primme_params *primme) {
   ...
   *restartSize += numPacked + *numPrevRetained;
   ...
   ret = Num_reset_update_VWXR_@(pre)primme(V, W, nLocal, basisSize, ldV, hVecs,
         *restartSize, ldhVecs, hVals,
         ...
         evecs, *numLocked+primme->numOrthoConst, left, *restartSize, primme->nLocal,
         W, 0, *restartSize, ldV,
   ...
   for (i=left; i < *restartSize; i++) {
       if (flags[i] != UNCONVERGED) {
         ...
         Num_copy_matrix_@(pre)primme(&evecs[(numLocked0+i-left+primme->numOrthoConst)*primme->nLocal], nLocal, 1,
