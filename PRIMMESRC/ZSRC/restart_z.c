...
int restart_zprimme(Complex_Z *V, Complex_Z *W, int nLocal, int basisSize, int ldV,
   ...
   if (!primme->locking) {
      ...
      Complex_Z *X, *Res;
      ret = restart_locking_zprimme(&restartSize, V, W, nLocal, R,
...
int Num_reset_update_VWXR_zprimme(Complex_Z *V, Complex_Z *W, int mV, int nV, int ldV,
   ...
   ret = ortho_zprimme(X0, ldX0, NULL, 0, nX2e-nX0b, nX0e-nX0b, evecs,
   ...
   if (H == NULL) {
      return max(
            compute_submatrix(NULL, numPrevRetained, 0, NULL, basisSize, 0, NULL,
               0, NULL, 0),
            solve_H_RR_zprimme(NULL, 0, NULL, 0, NULL, numPrevRetained,
...
static int restart_qr(Complex_Z *V, int ldV, Complex_Z *W, int ldW, Complex_Z *H,
   ...
   if (*targetShiftIndex < 0 || primme->targetShifts[*targetShiftIndex]
      ...
      *targetShiftIndex = min(primme->numTargetShifts-1, numConverged);
      ...
      if (QV) ret = update_projection_zprimme(Q, ldQ, V, ldV, QV, ldQV, nLocal, 0, restartSize,
            rwork, rworkSize, 0/*unsymmetric*/, primme);
      if (ret != 0) return ret;

      ret = solve_H_zprimme(H, restartSize, ldH, R, ldR, QV, ldQV, hU, newldhU, hVecs,
   ...
   if (QV) {
      Num_gemm_zprimme("N", "N", basisSize, restartSize, basisSize, tpone, QV, ldQV,
            hVecs, ldhVecs, tzero, rwork, basisSize);
      Num_copy_matrix_zprimme(rwork, basisSize, restartSize, basisSize, QV, ldQV);
   }
   ...
   Num_copy_matrix_zprimme(rwork, basisSize, numPrevRetained, basisSize,
   ...
   Num_geqrf_zprimme(basisSize, numPrevRetained, &R[ldR*indexOfPreviousVecs],
   ...
   Num_copy_matrix_zprimme(&R[ldR*indexOfPreviousVecs], basisSize,
      numPrevRetained, ldR, &hU[ldhU*indexOfPreviousVecs], ldhU);
   Num_orgqr_zprimme(basisSize, numPrevRetained, numPrevRetained,
   ...
   Num_copy_trimatrix_zprimme(&R[ldR*indexOfPreviousVecs], numPrevRetained,
   ...
   if (QV) {
      Num_gemm_zprimme("C", "N", restartSize, restartSize, basisSize, tpone, hU, ldhU, QV, ldQV,
            tzero, rwork, restartSize);
      Num_copy_matrix_zprimme(rwork, restartSize, restartSize, restartSize, QV, ldQV);
   }
   ...
   ret = solve_H_zprimme(&H[ldH*indexOfPreviousVecs+indexOfPreviousVecs], numPrevRetained, ldH,
